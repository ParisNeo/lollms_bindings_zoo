...
class mPLUG_Owl(LLMBinding):
    ...
    def __init__(self, 
        ...
        binding_config_template = ConfigTemplate([
            {"name":"device_map","type":"str","value":'auto','options':['auto', 'balanced', 'balanced_low_0', 'sequential'], "help":"Force using quantized version"},
    ...
    def build_model(self):
        ...
        if self.config.model_name:
            ...
            ASCIIColors.success(f"ok")
            ASCIIColors.info(f"Creating image processor {model_path}")
    ...
    def generate(self, 
        ...
        try:
            ...
            self.n_generated = 0
            self.output = ""
            if "images" in gpt_params:
                input_ids = self.processor(text=prompt, images=gpt_params["images"], return_tensors='pt').input_ids.to(self.model_device)
                del gpt_params["images"]
            else:
    ...
    def get_filenames(repo):
        ...
        main_url = "https://huggingface.co/"+repo+"/tree/main" 
    ...
    def download_model(repo, base_folder, callback=None):
        ...
        def download_file(get_file):
            main_url = "https://huggingface.co/"+repo
