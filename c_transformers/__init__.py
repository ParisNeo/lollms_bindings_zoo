...
class CTRansformers(LLMBinding):
    ...
    def build_model(self):
        ...
        if 'gpt2' in self.config['model_name']:
            ...
            model_type="gptq"
        elif 'llama' in self.config['model_name'].lower() or 'orca' in self.config['model_name'].lower() or'wizardlm' in self.config['model_name'].lower() or 'vigogne' in self.config['model_name'].lower() or 'ggml' in self.config['model_name'].lower() or'wizardcoder' in self.config['model_name'].lower():
            model_type='llama'
        else:
